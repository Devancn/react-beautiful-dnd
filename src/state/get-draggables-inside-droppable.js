...
export default memoizeOne(
  (
    ...
    const list = Object.keys(draggableDimensions).map(
      (key: Id): DraggableDimension => draggableDimensions[key]
    );
    return (
      list
        .filter(
          (dimension: DraggableDimension): boolean =>
        ...
        .sort(
          (a: DraggableDimension, b: DraggableDimension): number =>
            a.page.withoutMargin.center.y - b.page.withoutMargin.center.y
        )
    );
  }
);
